...
def ranking_correlation(y_out,true_val,node_num,model_size):
    ...
    order_result = heapq.nlargest(node_num, range(len(predict_arr)), predict_arr.__getitem__)
    assert(len(order_result) == node_num)

    kt, _ = kendalltau(predict_arr[:node_num], true_arr[:node_num])
    return kt

def loss_cal(y_out,true_val,num_nodes,device,model_size):
    top_30_num = int(0.3*num_nodes)
    ...
    ind_1 = torch.randint(0,top_30_num,(sample_num,)).long().to(device)
...
def mse_loss(y_out, true_val, device, model_size):
    y_out = y_out.reshape((model_size))
    true_val = true_val.reshape((model_size))
    loss = torch.nn.MSELoss(reduction='sum').forward(y_out, true_val)
